 ...
 */
bool NetworkService::sockIsValid(sock_t sock)
{
#if defined(_WIN32) || defined(__WIN32__) || defined (WIN32)
    ...
    /* Close the socket.
     */
    void NetworkService::killSock(sock_t sock)
    {
#if defined(_WIN32) || defined(__WIN32__) || defined (WIN32)
        closesocket(sock);
#else
    ...
    uint64_t NetworkService::getCurrentTimeMonotonic(void)
    {
        uint64_t time;
#if defined(_WIN32) || defined(__WIN32__) || defined (WIN32)
...
ip_ntoa(&((__ip_port__).ip)), ntohs((__ip_port__).port), 0, "OK", data_0(__buflen__, __buffer__), data_1(__buflen__, __buffer__));
     ...
     */
    int NetworkService::sendPacket(NetworkingCore *net, IP_Port ip_port, const uint8_t *data, uint16_t length)
    {
    ...
    void NetworkService::registerHandler(NetworkingCore *net, uint8_t byte, PacketHandlerCallback cb, void *object)
    {
        net->packethandlers[byte].function = cb;
        net->packethandlers[byte].object = object;
    }
    
    void NetworkService::poll(NetworkingCore *net)
    {
        if (net->family == 0) /* Socket not initialized */
            return;
        
        Utils::updateUnixTime();
...
#include <sodium.h>
    ...
    bool NetworkService::networkingAtStartup(void)
    {
        if (at_startup_ran != 0)
            return 0;
     ...
     */
    NetworkingCore * NetworkService::newNetworking(IP ip, uint16_t port)
    {
        return newNetworkingEx(ip, port, port + (TOX_PORTRANGE_TO - TOX_PORTRANGE_FROM), 0);
    }
     ...
     */
    NetworkingCore * NetworkService::newNetworkingEx(IP ip, uint16_t portFrom, uint16_t portTo, unsigned int *error)
    {
        ...
        if (portFrom == 0 && portTo == 0) {
            portFrom = TOX_PORTRANGE_FROM;
            portTo = TOX_PORTRANGE_TO;
        } else if (portFrom == 0 && portTo != 0) {
            portFrom = portTo;
        } else if (portFrom != 0 && portTo == 0) {
            portTo = portFrom;
        } else if (portFrom > portTo) {
            uint16_t temp = portFrom;
            portFrom = portTo;
            portTo = temp;
        }
        ...
        if (networkingAtStartup() != 0)
            return NULL;
        
        NetworkingCore *temp = (NetworkingCore*)calloc(1, sizeof(NetworkingCore));
         ...
         */
        uint16_t port_to_try = portFrom;
        *portptr = htons(port_to_try);
        int tries;
        
        for (tries = portFrom; tries <= portTo; tries++) {
            ...
            if (port_to_try > portTo)
                port_to_try = portFrom;
    ...
    void NetworkService::killNetworking(NetworkingCore *net)
    {
        if (!net)
            return;
     ...
     */
    int NetworkService::ipEqual(const IP *a, const IP *b)
    {
        if (!a || !b)
            return 0;
     ...
     * returns 0 when not equal or when uninitialized
     */
    int NetworkService::ipportEqual(const IP_Port *a, const IP_Port *b)
    {
        if (!a || !b)
            return 0;
    ...
    /* nulls out ip */
    void NetworkService::ipReset(IP *ip)
    {
        if (!ip)
            return;
    ...
    void NetworkService::ipInit(IP *ip, uint8_t ipv6enabled)
    {
        if (!ip)
            return;
    ...
    int NetworkService::ipIsset(const IP *ip)
    {
        if (!ip)
            return 0;
    ...
    /* checks if ip is valid */
    int NetworkService::ipportIsset(const IP_Port *ipport)
    {
        if (!ipport)
            return 0;
        ...
        return ipIsset(&ipport->ip);
    }
    
    /* copies an ip structure (careful about direction!) */
    void NetworkService::ipCopy(IP *target, const IP *source)
    {
        if (!source || !target)
            return;
    ...
    void NetworkService::ipportCopy(IP_Port *target, const IP_Port *source)
    {
        if (!source || !target)
            return;
    ...
    const char * NetworkService::ipNtoa(const IP *ip)
    {
        if (ip) {
     ...
     */
    int NetworkService::ipParseAddr(const IP *ip, char *address, size_t length)
    {
        if (!address || !ip) {
            return 0;
        }
     ...
     * returns 1 on success, 0 on failure
     */
    int NetworkService::addrParseIp(const char *address, IP *to)
    {
        if (!address || !to)
            return 0;
     ...
     */
    int NetworkService::addrResolve(const char *address, IP *to, IP *extra)
    {
        if (!address || !to)
            return 0;
        ...
        IP ip4;
        ipInit(&ip4, 0); // ipv6enabled = 0
        IP ip6;
        ipInit(&ip6, 1); // ipv6enabled = 1
     ...
     */
    int NetworkService::addrResolveOrParseIp(const char *address, IP *to, IP *extra)
    {
        if (!addrResolve(address, to, extra))
            if (!addrParseIp(address, to))
                return 0;
